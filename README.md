...
#### Решение
Для реализации нового подхода в хранении данных необходимо будет заменить запись в файлы в формате .json на модуль ProtocolBuffers, который отличается скоростью сохранения
данных.
#### Код
Данное решение находится на этапе реализации. Так как возник ряд трудностей как с реализацией нового подхода, так как в команде не было специалистов, работающих с ProtocolBuffers.
#### Оценка результативности
После реализации данного решенеия необходимо будет замерить время до использования нового метода записи в файл и после. Если время на запись в файл сократится, то данное решение улучшило показатели приложения, иначе стоит вернуться к записи в .json файл.
## Приложение
