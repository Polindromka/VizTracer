...
#### Оценка результативности
В результате удалось увеличить скорость почти в 1,5 раза (в 1,47). Для этого были произведены замеры до изменений и после:
![изображение](https://user-images.githubusercontent.com/59918228/172047491-b32e5da7-817f-4a67-93f7-d64f1aa4c05d.png) \\
Что позволило достичь поставленной цели - увеличить скорость записи в файл. Так как код приложения полностью зависит от .json формата, то использование  protobufer вызвало бы ряд трудностей, так как структура json'a всегда разная, нельзя определить какое-то n-ное кол-во структур, которые будут записываться в .json. Поэтому было принято решение остановиться на вышеупомянутом способе, так как он тоже давал значительный прирост вв скорости.
## Приложение
